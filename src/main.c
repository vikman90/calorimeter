...
int main() {
	...
	energy = askEnergy();
	intensity0 = sqrt(energy / (RESISTANCE * TIME_PULSE));
	initDevices();
	setIntensity(intensity0);
	...
	for (n = 0; n < ninyections; n++) {
		...
		for (j = 0; j < 2; j++) {
			for (i = 0; i < size; i++) {
				...
				intensity = intensity0 - sqrt(areaPartial / (K * RESISTANCE * latency));
				areas[n] += ABS(areaPartial);
				printf("\tNew intensity: %.5e\n", intensity);
